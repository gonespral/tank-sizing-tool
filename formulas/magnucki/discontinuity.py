"""
Calculates discontinuity stresses in a semi-ellipsoidal dome connected to a cylindrical shell.

Source: "Minimization of stress concentration factor in cylindrical pressure vessels with ellipsoidal heads" by
Magnucki et al.
"""
import numpy as np
...
def discontinuity_stress(p0: float, t1: float, t2: float, a: float, b: float, E: float, nu: float,
    ...
    sigmatheta1 = x1 * x2 / 2 / beta / (1 + xi * (np.sin(x)) ** 2) ** (1 / 2) - fthetaBD * np.exp(-psi)
    sigmatheta2 = x1 * x2 / 2 / beta / (1 + xi * (np.sin(x)) ** 2) ** (1 / 2) + fthetaBD * np.exp(-psi)
    ...
    fphiBD2 = sigmaM * (((cv ** 2) / 3 + nu) * np.cos(psi) - (cv ** 2) / 3 - nu) * np.cos(psi) - sigmatheta * (
                np.cos(psi) + 3 * nu / (cv ** 2) * np.sin(psi))
    sigmaphi1 = x1 * x2 / 2 / beta * (1 - xi * (np.sin(x)) ** 2) / (1 + xi * (np.sin(x)) ** 2) ** (
                1 / 2) - fphiBD1 * np.exp(-psi)
    sigmaphi2 = x1 * x2 / 2 / beta * (1 - xi * (np.sin(x)) ** 2) / (1 + xi * (np.sin(x)) ** 2) ** (
